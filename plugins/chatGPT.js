...
const handler = async (m, { conn, text, usedPrefix, command }) => {
  ...
  if (!text) throw `أهلاً يا ${taguser} بوت التربو بقى ذكي دلوقتي، لو عندك أي أسئلة أو استفسارات، اتفضل قول \n\n❏ مثال \n❏ ${usedPrefix + command} أحسن أنمي \n❏ ${usedPrefix + command} عايز نصيحة \n❏ ${usedPrefix + command} قول نكتة`;
  try {
    ...
    if (respuesta == 'error' || respuesta == '' || !respuesta) return XD; // لو حصل خطأ
    m.reply(`${respuesta}`.trim());
  } catch {
    try {
      conn.sendPresenceUpdate('composing', m.chat);
      const botIA222 = await openaiii.createCompletion({ model: 'text-davinci-003', prompt: text, temperature: 0.3, max_tokens: 4097, stop: ['Ai:', 'Human:'], top_p: 1, frequency_penalty: 0.2, presence_penalty: 0 });
      if (botIA222.data.choices[0].text == 'error' || botIA222.data.choices[0].text == '' || !botIA222.data.choices[0].text) return XD; // لو حصل خطأ
      ...
      try {
        ...
        if (fgjson1.result == 'error' || fgjson1.result == '' || !fgjson1.result) return XD; // لو حصل خطأ
        ...
        try {
          ...
          if (vihangaytjson1.data == 'error' || vihangaytjson1.data == '' || !vihangaytjson1.data) return XD; // لو حصل خطأ
          ...
          try {
            ...
            if (vihangaytjson2.data == 'error' || vihangaytjson2.data == '' || !vihangaytjson2.data) return XD; // لو حصل خطأ
            ...
            try {
              ...
              if (vihangaytjson3.data == 'error' || vihangaytjson3.data == '' || !vihangaytjson3.data) return XD; // لو حصل خطأ
              ...
              try {
                ...
                if (hasill22.result == 'error' || hasill22.result == '' || !hasill22.result) return XD; // لو حصل خطأ
                ...
                try {
                  ...
                  try {
                    ...
                    if (akuariapijson2.respon == 'error' || akuariapijson2.respon == '' || !akuariapijson2.respon) return XD; // لو حصل خطأ
                    ...
                    try {
                      ...
                      if (akuariapijson1.respon == 'error' || akuariapijson1.respon == '' || !akuariapijson1.respon) return XD; // لو حصل خطأ
